        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border rounded shadow">
                ...
                <tbody>
                            ...
                            @if ($user->role === 'teacher')
                                <td class="px-4 py-2 space-x-2">
                                    ...
                                    <form action="{{ route('grades.destroy', $grade) }}" method="POST" class="inline-block"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" aria-label="Dzēst šo atzīmi" class="text-red-600 hover:underline">Dzēst</button>
                                    </form>
                                </td>
                            @endif
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ $user->role === 'teacher' ? '4' : '3' }}" class="px-4 py-2 text-center">Atzīmes nav atrastas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
